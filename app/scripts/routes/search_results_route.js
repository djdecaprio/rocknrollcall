Rocknrollcall.SearchResultsRoute = Ember.Route.extend({
    model: function (query) {
        return Promise.all([$.getJSON("http://developer.echonest.com/api/v4/artist/search?api_key=CVHPXXVJB7A9QHCXF&format=json&results=10&bucket=images&bucket=hotttnesss&bucket=biographies&bucket=id:musicbrainz", { name: query.term }),$.getJSON("http://developer.echonest.com/api/v4/song/search?api_key=CVHPXXVJB7A9QHCXF&format=json&results=10&bucket=id:7digital-US&bucket=audio_summary&bucket=song_hotttnesss&bucket=tracks&bucket=song_type", { title: query.term }) ]).then(function(jsonArray){
